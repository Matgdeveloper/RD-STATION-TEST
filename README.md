...
Desenvolvido por [Matheus Gomes de Oliveira dos Santos]
...
Lógica de Recomendação (recommendation.service.js): Criei um sistema de pontuação (score) que soma os "matches" de preferências e funcionalidades. Para a regra de desempate, utilizei o índice original do array (originalIndex) para garantir que o último produto válido seja retornado. O código lida com entradas vazias retornando arrays limpos.

Integração com a UI: Utilizei o padrão de elevação de estado (Lifting State Up). O componente Form recebe a prop onSubmit e repassa os produtos filtrados para o componente pai App, que atualiza o estado geral e renderiza a RecommendationList.

Testes: Foram implementados testes unitários no Jest cobrindo o retorno nulo, cenário de SingleProduct, cenário de MultipleProducts e a regra específica de desempate.
